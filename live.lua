...
local lua_color = {r = 245, g = 245, b = 245} --204, 135, 255 rgb(142, 153, 255) rgb(176, 183, 255)
local data = obex_fetch and obex_fetch() or {username = 'BAURA', build = 'dev', discord=''}
...
local function downloadFile()
	http.get(string.format("https://cdn.discordapp.com/attachments/1291073365540601999/1294342996535414826/image.png?ex=670aaa5a&is=670958da&hm=0366d24e270f112012734950d426224c311f103238228ff2b591bdd909f47b5a&", MyPersonaAPI.GetMyCountryCode():lower()), function(success, response)
...
local function downloadFileLogo()
	http.get("https://cdn.sologo.ai/temp24h/logo/5303af61-7b58-40d5-a9a7-640e3c9cd5c0.png?x-oss-process=image/crop,x_0,y_900,w_400,h_300", function(success, response)
		if not success or response.status ~= 200 then
            return
		end
...
local ctx = new_class()
	:struct 'globals' {
		states = {"Stand", "Slow Walk", "Move", "Crouch", "Crouch-move", "air", "Aero + Crouch", "Fakelag"},
		extended_states = {"Global", "Stand", "Slow Walk", "Move", "Crouch", "Crouch-move", "air", "Aero + Crouch", "Fakelag"},
		teams = {"T", "CT"},
		in_ladder = 0,
		nade = 0,
		resolver_data = {}
	...
	:struct 'ui' {
		...
		execute = function(self)
			...
			self.menu.global.label = group:label(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. "\af5f5f5ff<\ab5b5b5FFBAURA\af5f5f5ff>")
			self.menu.global.label1 = group:label(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. " $ Welcome back. \aCC87FFFF" .. "\af5f5f5ff" .. login.username)
			self.menu.global.label2 = group:label(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. " $ Build loaded: \aCC87FFFF" .. "\af5f5f5ff" .. login.build)
			self.menu.global.label3 = group:label(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. "\af5f5f5ff<\ab5b5b5FFdev\af5f5f5ff>")
			...
			for _, team in ipairs(self.globals.teams) do
				...
				for _, state in ipairs(self.globals.extended_states) do
					...
					if state ~= "Global" then
						menu.enable = group:checkbox("Enable " .. func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. "\n" .. team)
					end

					menu.space323 = group:label("\n ".. state .. team)


					menu.pitchsetxd = group:combobox(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. " Pitch" .. "\n" .. state .. team, {"Off", "Default", "Minimal", "Up", "Custom"})
					menu.pitchSlider =  group:slider(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. " Custom" .. "\n" .. state .. team,  -89, 89, 0, true, "*", 1):depend({menu.pitchsetxd, "Custom", false})


					menu.yaw_base = group:combobox(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. " View" .. "\n" .. state .. team, {"local view", "at targets"})
                    menu.yaw_jitter = group:combobox(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. " Yaw" .. "\n" .. state .. team, {"off", "offset", "center", "random", "skitter"})
					menu.options = group:multiselect(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. " Yaw Extra" .. "\n" .. state .. team, {'Delayed'}):depend({menu.yaw_jitter, "off", true})
					menu.jitter_delay = group:slider(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. ' Delay' .. "\n" .. state .. team, 1, 4, 1, true, 'Â°', 1, {'1Â°'}):depend({menu.options, 'Delayed'})
					menu.yaw_add = group:slider(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. " Yaw (left)" .. "\n" .. state .. team, -180, 180, 0, true, "Â°", 1):depend({menu.yaw_jitter, "off", true})
					menu.yaw_add_r = group:slider(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. " Yaw (right)" .. "\n" .. state .. team, -180, 180, 0, true, "Â°", 1):depend({menu.yaw_jitter, "off", true})
                    menu.yaw_jitter_add = group:slider(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. " Amount" .. "\n" .. state .. team, -180, 180, 0, true, "Â°", 1):depend({menu.yaw_jitter, "off", true})

                    menu.yawCondition = group:combobox(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. " Yaw mode" .. "\n" .. state .. team, {"off", "Automatic"})
                    menu.yawCondition2 = group:combobox(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. " Tick-Based" .. "\n" .. state .. team, {"off", "on", "Automatic"})
					menu.tickbaseslider = group:slider(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. " Ticks" .. "\n" .. state .. team,  1, 4, 1, true, "-", 1):depend({menu.yawCondition2, "Automatic", true}, {menu.yawCondition2, "off", true})
                    menu.micromovements = group:checkbox(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. " Micromovement Correction" .. "\n" .. state .. team):depend({menu.yawCondition, "off", true})
                    menu.micromovements2 = group:checkbox(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. " Body Correction" .. "\n" .. state .. team):depend({menu.yawCondition, "off", true})

                   -- menu.yaw_add3 = group:slider(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. "Yaw (left)" .. "\n" .. state .. team, -180, 180, 0, true, "Â°", 1):depend({menu.yawCondition, "off", true})
					--menu.yaw_add_r3 = group:slider(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. "Yaw (right)" .. "\n" .. state .. team, -180, 180, 0, true, "Â°", 1):depend({menu.yawCondition, "off", true})
                   -- menu.yaw_jitter_add3 = group:slider(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. "Amount" .. "\n" .. state .. team, -180, 180, 0, true, "Â°", 1):depend({menu.yawCondition, "L & R" and "Tickbased", false})
                   -- menu.yaw_delay = group:slider(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. "Delay" .. "\n" .. state .. team, -180, 180, 0, true, "Â°", 1):depend({menu.yawCondition, "Delay", false})

                    menu.desync_mode = group:combobox(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. " Body yaw" .. '\n' .. state .. team, {'Default', 'Custom Desync'})
					menu.body_yaw = group:combobox("\n XD" .. "\n" .. state .. team, {"off", "static", "opposite", "jitter"})
					menu.body_yaw_side = group:combobox(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. ' Side' .. "\n" .. state .. team, {'left', 'right', 'freestanding'}):depend({menu.body_yaw, "static", false})
                    

                    menu.defensive_enable = group:checkbox(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. " Enable Defensive" .. "\n" .. state .. team)
                    menu.defensive_statekurwa = group:multiselect("\ndefensive yaw mode" .. "\n" .. state .. team, {'Defensive Builder'}):depend({menu.defensive_enable, true})


					--"Stand", "Move", "Crouch", "air", "Aero"
                    menu.defensive_conditions = group:multiselect(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. "Triggers" .. "\n" .. state .. team, {'Always on', 'Weapon switch', 'Reload', 'Damage', 'On Freestanding'}):depend({menu.defensive_statekurwa, 'Defensive Builder'}, {menu.defensive_enable, true})
					menu.defensive_yaw = group:checkbox(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. "Defensive Yaw" .. "\n" .. state .. team):depend({menu.defensive_statekurwa, 'Defensive Builder'},{menu.defensive_enable, true})
					menu.defensive_yaw_mode = group:combobox("\ndefensive yaw mode" .. "\n" .. state .. team, {'Pitch Exploit', 'Spin', "Jitter", "Flick", "Fake Up", "BAURA"}):depend({menu.defensive_statekurwa, 'Defensive Builder'}, {menu.defensive_yaw, true}, {menu.defensive_enable, true})
					menu.defensive_freestand = group:checkbox(func.hex({lua_color.r, lua_color.g, lua_color.b}) .. state .. "\af5f5f5ff" .. "Avoid Yaw" .. "\n" .. state .. team):depend({menu.defensive_statekurwa, 'Defensive Builder'}, {menu.defensive_enable, true})
					--"Stand", "Move", "Crouch", "air", "Aero"
			...
			self.menu.Antiaim.import = group2:button("import", function ()
				...
				local type = data:match("{BAURA:(.+)}")
						self.config:import(data, type, self.menu.Antiaim.team:get(), self.menu.Antiaim.state:get())
			end):depend({self.menu.Antiaim.mode, "Builder"})

			--Misc
			self.menu.Antiaim.freestanding = group:multiselect("freest\af5f5f5ffanding", {"disablers", "static freestand"}, 0x0):depend({self.menu.Antiaim.mode, "Keybinds"})
			...
			self.menu.Antiaim.edge_yaw = group:label("Edge\af5f5f5ffYaw", 0x0):depend({self.menu.Antiaim.mode, "Keybinds"})
			self.menu.Antiaim.manual_left = group:hotkey("Manual " .. "\af5f5f5ff" .. "\af5f5f5ffleft"):depend({self.menu.Antiaim.mode, "Keybinds"})
			self.menu.Antiaim.manual_right = group:hotkey("Manual " .. "\af5f5f5ff" .. "\af5f5f5ffright"):depend({self.menu.Antiaim.mode, "Keybinds"})
			self.menu.Antiaim.manual_forward = group:hotkey("Manual " .. "\af5f5f5ff" .. "\af5f5f5ffforward"):depend({self.menu.Antiaim.mode, "Keybinds"})
           -- self.menu.Antiaim.spacing = group:label(" ")

		    self.menu.Misc.otheraa = group:multiselect("Extra", {'Anti backstab', 'Safe head'})
			self.menu.Misc.resolver = group:checkbox("AI \af5f5f5ffResolver")
			self.menu.Misc.resikmode = group:combobox("Mo\af5f5f5ffde", {"Default", "New"}):depend({self.menu.Misc.resolver, true})
            self.menu.Misc.resolverselector = group:multiselect("Sel\af5f5f5ffect", {"BAURA corections", "Delay", "Missmatch"}):depend({self.menu.Misc.resolver, true})

			self.menu.Misc.enablepeek = group2:checkbox("Enable \af5f5f5ffAIpeek")
			self.menu.Misc.aipeek = group2:hotkey("\af5f5f5ff[AI]\r peek", 0x0):depend({self.menu.Misc.enablepeek, true})
			self.menu.Misc.aisettings = group2:combobox("Mode", {"Smart", "Aggressive"}):depend({self.menu.Misc.enablepeek, true})

			self.menu.Misc.animations = group:checkbox("Local\af5f5f5ff Animations")
			self.menu.Misc.animations_selector = group:multiselect("Sel\af5f5f5ffect ", {"Break legs while in Aero", "Reversed legs", "Freeze", "Reset pitch on land", "Micheal Jackson"}):depend({self.menu.Misc.animations, true})
			
			self.menu.Misc.trashtalk = group:checkbox("Trashtalk")
			self.menu.Misc.trashtalkmode = group:combobox("Sel\af5f5f5ffect ", {"Default", "Spanish", "Romanian"}):depend({self.menu.Misc.trashtalk, true})
			self.menu.Misc.fastladder = group:multiselect("Fast\af5f5f5ffladder", {"180", "Ascending", "Descending"})
			...
			self.menu.Visuals.indicators = group:checkbox("Enable \af5f5f5ffIndicators", {140, 125, 255})
            self.menu.Visuals.indicatorstyle = group:combobox("Indicator\af5f5f5ff Style", {"Default", "New",}):depend({self.menu.Visuals.indicators, true})
			...
			self.menu.Visuals.screenlogs = group:checkbox("Enable\af5f5f5ff Screen Logs", {140, 125, 255})
			self.menu.Visuals.screenlogsstyle = group:combobox("Logs\af5f5f5ff Style", {"Default", "Normal"}):depend({self.menu.Visuals.screenlogs, true})

            self.menu.Visuals.xd = group:multiselect("Other\af5f5f5ff Indicators", {"Watermark", "Defensive Indicator", "Slowdown Indicator"})
			...
			self.menu.Visuals.debug_panel = group:combobox("Debug \af5f5f5ffPanel", {"Off", "#1", "#2", "#3"})
      ...
      return closest
		...
		get_state = function(self)
			...
			if self:in_air(me) or self.was_in_air then
				state = duck and "Aero + Crouch" or "air"
	...
	:struct 'config' {
		...
		update_configs = function (self)
			...
			self:write_file("BAURA_configs.txt", self.configs)
			self:update_name()
		end,

		setup = function (self)
			local data = readfile('BAURA_configs.txt')
			if data == nil then
				self.configs = {}
				return
			end
		...
		import = function (self, data, type, ...)
			local name = data:match("{BAURA:(.+)}")
			if not name or name ~= type then
				return error('This is not valid BAURA data. 1')
			end

			local success, err = pcall(self['import_'..name], self, data:gsub("{BAURA:" .. name .. "}:", ""), ...)
			if not success then
				print(err)
				return error('This is not valid BAURA data. 2')
			end
		end,
			...
			if data.defensive_yaw and data.defensive_enable and self.helpers:contains(data.defensive_statekurwa, 'Defensive Builder') then
        	  ...
        	  if defensive > 0 then
                ...
                elseif data.defensive_yaw_mode == 'Fake Up' and defensive > 0 then
					yaw_offset = 5
					pitch = -87
				elseif data.defensive_yaw_mode == "BAURA" and defensive > 0 then
					local tickcount = globals.tickcount()
					yaw_offset = tickcount % 3 == 0 and client.random_int(90, -90) or tickcount % 3 == 1 and 180 or tickcount % 3 == 2 and client.random_int(-90, 90) or 0
    ...
    render_crosshair = function (self)
		...
		render = function(self)
            ...
            local strike_w, strike_h = renderer.measure_text(size, func"BAURA")
...
--		self.menu.Visuals.watermark = group:combobox("Style", {"Default", "Country-based"}):depend({self.menu.Visuals.xd, "Watermark"})
			...
			if self.ui.menu.Visuals.xd:get('Watermark') then
				if watermarkstylexd == "Modern" then
					...
					if wtr_img ~= nil and download ~= nil then
						local mainY = 35
						local marginX, marginY = renderer.measure_text("-d","BAURA.DEV")
						...
						renderer.text(33, sizeY/2 - 2 + mainY, 255, 255, 255, 255, "-d", nil, "BAURA" .. ".DEV")
						renderer.text(33, sizeY/2 - 4 + marginY + mainY, 255, 255, 255, 255, "-d", nil, "[ dev͙֒ ]")
					else
						downloadFile()
					end
				elseif watermarkstylexd == "Default" then
					local r,g,b = self.ui.menu.Visuals.indicators:get_color()
					local text = "\a"..rgba_to_hex(255,255,255, 255).." ♡ B A U "..text_fade_animation_guwno(2,255,255,255,255,"R A ") .."\a"..rgba_to_hex(255, 255, 255, 255).." ["..string.upper(login.build).. "]"
				...
				if not self.ui.menu.Visuals.indicators:get() then
					renderer.text(self.ss.x/2, self.ss.y - 14, 255, 255, 255, 255, "cb" .. "cb", nil, "Baura🌷")
				end
			end

			if self.ui.menu.Visuals.debug_panel:get() ~= "Off" then
				local pos_wtm = {5, self.ss.y /2 - 50}
				if self.ui.menu.Visuals.debug_panel:get() == "#1" then
					local guwno_x, guwno_y = renderer.measure_text("-d",string.upper("BAURA    ["..login.build.."]    |    "..login.username.."    |    "..round(client.latency()*1000, 0).."ms"))
					...
					renderer.text(pos_wtm[1], pos_wtm[2], 255,255,255,255, "-d", nil, string.upper("BAURA🌷͙֒    ["..login.build.."]    |    "..login.username.."    |    "..round(client.latency()*1000, 0).."ms"))
					--140, 125, 255
					renderer.gradient(pos_wtm[1], pos_wtm[2] + 13, guwno_x / 2 -1, 1, 100, 125, 255,0,140, 125, 255,255, true)
				...
				if self.ui.menu.Visuals.debug_panel:get() == "#2" then
					renderer.text(pos_wtm[1], pos_wtm[2] + 4, 255, 255, 255, 255, "", 0, "BAURA.DEV🌷͙֒ - " .. string.lower(login.username))
				...
				if self.ui.menu.Visuals.debug_panel:get() == "#3" then
					...
					--local debug_pnl_x_text, debug_pnl_y_text = renderer.measure_text("",gradient_text("BAURA.pub", {255,30,90,255}, 10))
					...
					renderer.text(pos_wtm[1] + 4, pos_wtm[2] + 70 + 3, 255, 255, 255, 255, "", 0, ">> 🌷͙֒BAURA🌷͙֒")
      ...
      if style == "Default" then
        ...
        local visual_size = self.ui.menu.Visuals.indicatorfont:get()
			  ...
			  local strike_w, strike_h = renderer.measure_text(size, func"BAURA dev")
			  ...
			  renderer.text(self.ss.x/2 + ((strike_w + 2)/2) * scoped_frac, self.ss.y/2 + 20, 255, 255, 255, 255-(150*self.weapon_fraction), size .. "c", 0, func"BAURA ", func("\a" .. self.helpers:rgba_to_hex( r, g, b, ((255-(150*self.weapon_fraction)) * math.abs(math.cos(globals.curtime()*2)))) .. "dev"))
    ...
    --
        renderer.text(self.ss.x/2 + 27 * scoped_frac, self.ss.y/2 + 15, 0, 0, 0, 50, "c-", 0, gradient_text("BAURA.DEV", {r,g,b,255}, 10))
	...
	--
	  ...
	  elseif style == "Second" then
		local ref = {
			...
			safe = ui.reference("rage", "aimbot", "force safoint"),
		...
		animated_text(self.ss.x/2 - renderer.measure_text("b", "BAURA")/2 + 23* scoped_frac , y/2 + 15, 7, {r=clr_r, g=clr_g, b=clr_b, a=255}, {r=50, g=50, b=50, a=255}, "b", "BAURA")
	...
	local kill = {
		...
		"1",
		"ez by bianca",
		"why you miss to BAURA",
		"go buy cfg by viro",
		"killed by viro so shit",
		"Go download cfg by lightleaks",
		"BY LORDVIRO",
		"BY LORDBIANCA",
		"Owned by lightleaks",
		"discord.gg/lightleak join here for better cfg and lua",
		"keep missing BAURA.DEV",
	...
	local Spanish = {
        ...
        "PequeÃ±o maricÃ³n",
        "Que Dios te dÃ© brazos cortos para que tique el culo",
        ...
        "Si tongo el pene en el hombro, dirÃ¡s que llevabas troncos a casa",
        ...
        "no treocupes por tu boca muerta",
	...
	local Romanian = {
        ...
        "DÄƒ-le dracului dÄƒreri, o, Doamne",
        ...
        "Du-te dracului pÃ¢nÄƒ Ã®È›i ies caramele fund",
        "Preotul È™tie cÄƒ tlimbi dezgropat?",
        ...
        "Te caci pietre de cale feratÄƒ",
...
--             clan_tag = clantag_anim('BAURA', {13})
...
client.set_event_callback('paint_ui', function()
	...
	if renderguwno == "yes" then
		...
		renderer.text(screen.x/2, screen.y/2 + 40, 184, 184, 184, alpha, 'c', 0, '       Loading BAURA\nWelcome - '..login.username..'['..login.build..']')
...
client.set_event_callback('paint_ui', function()
	...
	renderer.text(screen.x/10, screen.y/3, 200, 200, 200, 355, 'c', 0, 'You have chance')
	...
	if renderguwno == "yes" then
		...
		renderer.text(screen.x/2, screen.y - y, 184, 184, 184, alpha, 'c', 0, 'Me love you for using lua)')
